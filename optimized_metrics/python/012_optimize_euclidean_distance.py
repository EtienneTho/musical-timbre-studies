...
###
### This script computes the euclidean distances between the representations of sounds of the different datasets defined in tsp (line 407)
### The results are logged in the "log_foldername" folder (line 371), must be changed for each optimization to not erase former results
### The representation to consider is defined by the  "reduce_strf" value (line 372) 
### and can take the following values: 'avg_time': averaged over time, 'avg_time_avg_freq': averaged over time and frequency (scale-rate),
...
### 'avg_time_all11': averaged over time decimated at 10x10x10
###
### This script replicates the results of the section "Optimized metrics simulating human dissimilarity ratings" for the euclidean distances.
###
...
def run_one_tsp_rs(tsp, rs, args):
    ...
    if rs_type == 'strf':
        if reduce_strf == 'pca':
            ...
            for i in range(len(aud_repres)):
                ...
                tab_red.append(strf_avgTime.flatten()) # decimation here
            ...
            for i in range(len(aud_repres)):
                ...
                # avgvec2strfavg(strf2avgvec(aud_repres[i]))
            ...
            for i in range(len(aud_repres)):
                ...
                # tab_red.append(np.absolute(np.mean(aud_repres[i], axis=0)).flatten())
            tab_red = np.transpose(np.asarray(tab_red))
        elif reduce_strf == 'avg_time_all11':
            for i in range(len(aud_repres)):
                # print(np.mean(np.abs(aud_repres[i]), axis=0).flatten().shape)
                # print(sg.decimate(np.mean(np.abs(aud_repres[i]), axis=0).flatten(),4).shape)
                strf_avgTime = np.mean(np.abs(aud_repres[i]), axis=0)

                # print(strf_avgTime[0:128:12,0:11,0:22:2].shape)[0:128:12,0:11,0:22:2]
                tab_red.append(strf_avgTime[0:128:12,0:11,0:22:2].flatten())               
                # strf_scale_rate, strf_freq_rate, strf_freq_scale = avgvec2strfavg(strf2avgvec(aud_repres[i]))
                # plotStrfavg(strf_scale_rate, strf_freq_rate, strf_freq_scale)
                # tab_red.append(np.absolute(np.mean(aud_repres[i], axis=0)).flatten())
            tab_red = np.transpose(np.asarray(tab_red))            
    ...
    correlations, p, lower, upper, power, df = training.euclidean_distance_corr(tab_red, dissimil_mat)
    ...
    pickle.dump({'representations': tab_red,
                 'dissimilarities': dissimil_mat,
                 'audio_args': args['audio_args'],
...
def run(tsp=None, rs=None):
    ...
    args = {
        ...
        'log_foldername': './out_aud_STRF_euclidean_distance_avg_time_all11_WithFullStats/',
        'reduce_strf': 'avg_time_all11',
        'test_args': {
            'snd_crossval': False
        },
        'audio_args': {
...
if __name__ == '__main__':
    ...
    run(tsp=['Barthet2010','Grey1977','Grey1978','Iverson1993_Onset',
...
# tsp=['Barthet2010','Grey1977','Grey1978','Iverson1993_Onset',
#         'Iverson1993_Remainder','Iverson1993_Whole', 'McAdams1995','Patil2012_A3',
#              'Patil2012_DX4','Patil2012_GD4','Siedenburg2016_e3', 'Lakatos2000_Harm','Lakatos2000_Comb','Lakatos2000_Perc','Siedenburg2016_e2set1'
#              ,'Siedenburg2016_e2set2','Siedenburg2016_e2set3']

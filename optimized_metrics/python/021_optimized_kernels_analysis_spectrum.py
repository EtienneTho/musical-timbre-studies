...
###
### This script analyzes the optimized metrics obtained with 01X_XXX scripts for the spectrum only
...
sigmasPath = ['out_folder_spectrum']
    ...
    for r, d, f in os.walk(path):
     for file in sorted(f):
      if 'resultsOptims' in file: # 
       ...
       # print(file+' '+str(corr_value_explained_variance))
...
np.savetxt('test.txt', np.transpose(np.asarray(tabCorrOrganised)), delimiter=';', fmt='%.2f')

...
def euclidean_distance_corr(input_data,
                       target_data,
                       log_foldername='./'):
    ...
    kernel = np.zeros((ninstrus, ninstrus))
    idx = [i for i in range(len(input_data))]
    print(input_data[:,10].shape)
    for i in range(ninstrus):
            for j in range(i + 1, ninstrus):
                i_tab = np.abs(input_data[:,i]) / np.abs(np.max(input_data[:,i]))
                j_tab = np.abs(input_data[:,j]) / np.abs(np.max(input_data[:,j]))
                # kernel[i, j] = np.sqrt(np.sum((i_tab-j_tab)**2))
                kernel[i, j] = np.abs(pearsonr(input_data[:,i], input_data[:,j])[0])
    kernel_v = kernel[idx_triu]
    mean_kernel = np.mean(kernel_v)
    std_kernel = np.std(kernel_v)
    Jn = np.sum(np.multiply(kernel_v - mean_kernel, target_v - mean_target))
